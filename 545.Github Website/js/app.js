...
window.updateProfile = function (profileUrl) {
  ...
  fetchData(
    ...
    (data) => {
      ...
      <a href="${githubPage}" targe="_blank" class="btn btn-secondary">
      ...
      ${
        website
          ...
          <a href="${website}" class="meta-text">${website.replace(
              "https://",
              ""
            )}</a>
        </li>
        ...
        twitter_username
          ...
          <span class="icon">
            ...
            >
              ...
              ></path></svg
          ></span>
          <a href="https://twitter.com/${twitter_username}" class="meta-text">@${twitter_username}</a>
        </li>`
          : ""
      }
      </ul>
...
const /** {NodeElement} */ $forkedPanel =
    document.querySelector("[data-fork-panel]");
const /** {NodeElement} */ $forkedTabBtn = document.querySelector(
    "[data-forked-tab-btn]"
  );
...
$forkedTabBtn.addEventListener("click", updateForkRepo);
...
const /** {NodeElement} */ $followerPannel = document.querySelector(
    "[data-follower-panel]"
  );

const updateFollower = function () {
  $followerPannel.innerHTML = `
  ...
  fetchData(followerUrl, function (data) {
    $followerPannel.innerHTML = `<h2 class="sr-only">Followers</h2>`;
    if (data.length) {
      for (const item of data) {
        ...
        `;
        $followerPannel.appendChild($followerCard);
      }
    } else {
      $followerPannel.innerHTML = `
    <div class="error-content">
...
const updateFollowing = function () {
  ...
  fetchData(followingUrl, function (data) {
    ...
    <div class="error-content">
      <p class="title-1">Oops! :(</p>
      <p class="text">Doesn't have any follwing yet.</p>
    </div>`;
    }
  });
};
