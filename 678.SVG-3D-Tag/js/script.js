    ...
    function t(t, e) {
        ...
        function i() {
            var e = window.innerWidth || document.documentElement.clientWidth || document.body.clientWidth,
                ...
                i = e,
                r = n;
            p.width.toString().indexOf("%") > 0 || p.height.toString().indexOf("%") > 0
                ? ((i = Math.round((t.offsetWidth / 100) * parseInt(p.width))), (r = Math.round((i / 100) * parseInt(p.height))))
        ...
        function l() {
            ...
            for (var o = 0, r = S.length; r > o; o++) {
                ...
                var l;
                M
                    ? ((l = (x - s.vectorPosition.z) / w), l < p.opacityOut && (l = p.opacityOut))
                    : ((l = parseFloat(s.element.getAttribute("opacity"))), (l += s.mouseOver ? (p.opacityOver - l) / p.opacitySpeed : (p.opacityOut - l) / p.opacitySpeed)),
...
$( document ).ready( function() {
            ...
            var entries = [ 
                ...
                { label: 'svg dual icons', url: '', target: '_blank' },
                { label: 'pop up menu', url: '', target: '_blank' },
                { label: 'Menu Design', url: '', target: '_blank' },
                { label: 'bubbles animation', url: '', target: '_blank' },
                { label: 'Dropdown Menu', url: '', target: '_blank' },
                { label: '3D', url: '', target: '_blank' },
                { label: 'happy birthday', url: '', target: '_blank' },
                { label: 'dashboard', url: '', target: '_blank' },
                { label: 'ui Kit', url: '', target: '_blank' },
                { label: 'fish animation', url: '', target: '_blank' },
                { label: 'Galaxy', url: '', target: '_blank' },
                { label: 'Error 404', url: '', target: '_blank' },
                { label: 'Lord Ganesha', url: 'https://codepen.io/uiswarup/details/abNBGKL', target: '_blank' },
                { label: 'Prcing Table', url: '', target: '_blank' },
                { label: 'New Login', url: '', target: '_blank' },
                { label: 'Creative Section Design', url: '', target: '_blank' },
                { label: 'svg loader', url: '', target: '_blank' },
                { label: '404 Error', url: '', target: '_blank' },
                { label: 'dust particle', url: '', target: '_blank' },
                { label: 'Creative Hexagon with Hover Effect', url: '', target: '_top' },
                { label: 'Birthday Card', url: '', target: '_top' },
                { label: 'Alert Message', url: '', target: '_top' },
                { label: 'happy new year', url: '', target: '_top' },
                { label: 'menu with awesome hover', url: '', target: '_top' },
                { label: 'Mobile Menu', url: '', target: '_top' },
                { label: 'awesome slider', url: '', target: '_top' },
                { label: 'Animated Background', url: '', target: '_blank' },
                { label: 'border-animation-css', url: '', target: '_blank' }
