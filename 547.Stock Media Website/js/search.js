...
const /** {NodeElement} */ $searchCleanBtn = document.querySelector(
    "[data-search-clear-btn]"
  );

$searchCleanBtn.addEventListener("click", () => ($searchField.value = ""));
...
const /** {NodeElement} */ $activeSegmentBgn = document.querySelector(
    "[data-segment-btn].selected"
  );
window.searchType = $activeSegmentBgn.dataset.segmentValue;
...
$searchBtn.addEventListener("click", function () {
  const /** {String} */ searchValue = $searchField.value.trim();
  if (searchValue) {
    updateSearchHistory(searchValue);
    window.filterObj.query = searchValue;
    updeteUrl(window.filterObj, window.searchType);
  }
});
