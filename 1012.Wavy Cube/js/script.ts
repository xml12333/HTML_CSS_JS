...
import type { Distance, Matrix, Vector3 } from "./types";

class WavyCube {
	private readonly canvas?: HTMLCanvasElement | null;
	private readonly ctx?: CanvasRenderingContext2D | null;
	private readonly cubeSize = 320;
	private readonly edgeFadeWidth = 15;
	private readonly lightDir = [0, 0, 1];
	private readonly lineCount = 16;
	private readonly lineSpacing = 40;
	private readonly lineWidth = 2;
	private readonly maxAmplitude = 32;
	private readonly rotationSpeed = 0.02;
	private readonly themeQuery: MediaQueryList = window.matchMedia(
		"(prefers-color-scheme: dark)"
	);
	private readonly waveFrequency = 0.5;
	 ...
	 * @param m Matrix
	 */
	private applyMatrix(v: Vector3, m: Matrix): Vector3 {
		return [
	...
	private draw(): void {
		...
		for (let i = 0; i < this.lineCount; i++) {
			...
			for (let screenX = 0; screenX < this.width; screenX++) {
				...
				const rayOrigin: Vector3 = [cx, cy, cz];
				const rayDir: Vector3 = [0, 0, -1];
				...
				if (hit) {
					...
					*/
					...
					let intensity = this.dot(sceneNormal, this.lightDir as Vector3);
					/*
	 ...
	 * @param ry Y radius
	 */
	private getRotationMatrix(rx: number, ry: number): Matrix {
	 ...
	 */
	private intersectBox(origin: Vector3, dir: Vector3, size: number): Distance | null {
		const tMinRef: Distance = {
			t: -Infinity,
			normal: [0, 0, 0]
		};
		const tMaxRef: Distance = {
			t: Infinity
		};
		// check X slabs
	...
	private intersectSlab(
		...
		if (Math.abs(dirAxis) < 1e-5) {
			...
			let n1: Vector3 = [0, 0, 0];
			let n2: Vector3 = [0, 0, 0];
